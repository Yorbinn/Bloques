...
#full_name = Proc.new do |first, last|
 # puts "#{last}, #{first}"
#end
#full_name.call('George', 'Tejeda')

full_name = Proc.new do |first: name, last: surname|
  puts "#{last}, #{first}"
end
full_name.call(first: 'George', last: 'Tejeda')
full_name.call(first: 'Tejeda', last: 'George')
...
def load_users(database, &block)
  ...
  block.call(first: 'George', last: 'Tejeda')
end
load_users('users.sqlite3', &full_name)

#---------------------------------------------------
users = ['Maria', 'Ramon', 'Victor', 'Juana']
def last_user(users)
...
full_name = Proc.new do |first:'Maria', last: 'Sanches', **list|
...
full_name = lambda do |first:'Maria', last: 'Sanches', **list|
  puts "#{last}, #{first}: #{list}"
end
